...
1. 工作台导出或发送企业微信时，末尾黄色填写字段为“系统-PCC：恢复编码”，例如`BSP-SZX173：PNR8X2`；
...
4. 差异说明、实际责任人、申诉状态、申诉结果和结案处理结果按业务回传覆盖到底稿；导出时申诉原因保持空白，业务填写后在转换时合并到备注；
5. 当前阶段、确认结果、转单信息、处理进度、申诉原因、系统、PCC、恢复编码及其当前跟进状态按字段合并到`备注`，旧工作台备注中未被本次填写的字段继续保留；
6. 下载生成的38列Excel，核对后导入ADM管理系统。

恢复状态取本服务恢复跟进库的实时记录，输出为待恢复、恢复中、已完成或异常。Excel有编码但尚未导入跟进库时标记“未进入恢复跟进”；Excel编码与跟进库编码不一致时在备注中同时写明跟进库编码和状态，避免把旧编码误认为已完成。

状态转换口径：“是否确认”为`确认`时转为待审核/不申诉；为`资料不全`或`非我司订单`时转为待申诉。未填写确认结果时，`待提交`转为待申诉；`已提交`转为申诉中；`不提交可结案`、申诉成功和申诉失败转为待审核。转换不会自动结案，仍由ADM专员在原系统审核。若数据库找不到ADM、同一ADM存在冲突行或业务字段超过明确长度，整次转换停止，不会静默丢行。历史备注按真实数据完整保留，不再按旧版200字表结构描述进行拦截或截断。
