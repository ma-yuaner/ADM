...
1. 工作台导出或发送企业微信时，Excel最后一列为黄色“恢复编码”；
...
5. 当前阶段、确认结果、转单信息、处理进度和恢复编码合并到`备注`；
6. 下载生成的38列Excel，核对后导入ADM管理系统。

状态转换口径：`待提交`转为待申诉；`已提交`转为申诉中；`不提交可结案`、申诉成功和申诉失败转为待审核。转换不会自动结案，仍由ADM专员在原系统审核。若数据库找不到ADM、同一ADM存在冲突行或目标字段超过数据库长度，整次转换停止，不会静默丢行。
