...
def test_wecom_can_send_to_group_without_person_mapping(monkeypatch):
    people_file = Path(__file__).resolve().parents[1] / "config" / "wecom_people.json"
    calls = []
    ...
    def fake_post(url, **kwargs):
        calls.append((url, kwargs))
        if "upload_media" in url:
            return FakeResponse({"errcode": 0, "media_id": "media-2"})
        return FakeResponse({"errcode": 0, "errmsg": "ok"})
    ...
    result = service.send_excel("未映射人员", BytesIO(b"xlsx"), "ADM.xlsx", 2)

    assert result["mentioned"] is False
    assert calls[0][1]["json"]["text"]["mentioned_list"] == []
    assert calls[0][1]["json"]["text"]["mentioned_mobile_list"] == []
